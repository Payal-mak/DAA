...
vvi make_adj_matrix_ud_w(pair<int, vector<vector<int>>> &graph_w)
{
    int v = graph_w.first;
...
vvi make_adj_matrix_d_w(pair<int, vector<vector<int>>> &graph_w)
{
    int v = graph_w.first;
    vvi adj_matrix(v, vi(v, -1));
    for (vector<int> &edge : graph_w.second)
    {
        // int start = edge[0], end = edge[1];
        adj_matrix[edge[0]][edge[1]] = edge[2];
...
{
    auto graph_w = dummy_graph();
    auto adj = make_adj_matrix_ud_w(graph_w);
